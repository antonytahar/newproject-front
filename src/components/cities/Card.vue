      ...
      <form>
        <div class="form-group col-lg-6 col-sm-12">
          <div class="form-group row"> <!-- Code Postal -->
            ...
            <input v-model="form.post_code" type="number" class="col-9 form-control" id="post_code">
          </div>
...
export default {
  ...
  computed: { // Au moment ou la page à finie de charger
    city () {
      return this.$store.getters['city/item']
    },
    countries () {
  ...
  mounted () { // Lorsque le HTML est interpreté
    this.$store.dispatch('city/fetchCityById', this.$route.params.cityid)
  ...
  methods: {
    ...
    submit: function () {
      this.$store.dispatch('city/saveCity', {no: this.$route.params.cityid, data: this.form})
    }
  }
}
</script>
